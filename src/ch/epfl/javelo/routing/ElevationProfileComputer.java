...
/**
 * Elevation profile utility class to compute the full and complete profile
 *
...
public final class ElevationProfileComputer {
     ...
     * @throws IllegalArgumentException if the maxStepLength isn't strictly positive
     */
    public static ElevationProfile elevationProfile(Route route, double maxStepLength) {
        ...
        //Case If only NaN
        int counter = 0;
        for (float v : edgesElevations2) {
            if (Float.isNaN(v)) {
                counter++;
            }
        }
        if (counter == edgesElevations2.length) {
            Arrays.fill(edgesElevations2, 0);
        }
